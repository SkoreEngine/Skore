...
/** Wire form for vector/quat/color/mat fields (contract §5): an array of f64
 * values in field order; the repository blob stores f32 components. */
static void write_f32_seq_named(sk_archive_writer_t* writer, sk_str_view_t name, const f32* comps, u32 count) {
	writer->begin_seq_named(writer->instance, name);
	for (u32 i = 0u; i < count; ++i) {
		writer->add_float(writer->instance, (f64)comps[i]);
	}
	writer->end_seq(writer->instance);
}

/** Read a named f64 sequence (vector/quat/color/mat wire form) into @p out
 * (up to @p cap entries; excess entries are drained). Returns the number of
 * entries read, or 0 when the named field is absent. */
static u32 read_f64_seq_named(sk_archive_reader_t* reader, sk_str_view_t name, f64* out, u32 cap) {
	if (!reader->begin_seq_named(reader->instance, name)) {
		return 0u;
	}
	u32 n = 0u;
	while (n < cap && reader->next_seq_entry(reader->instance)) {
		out[n++] = reader->get_float(reader->instance);
	}
	while (reader->next_seq_entry(reader->instance)) {
		/* drain excess entries */
	}
...
static i32 write_field_value(sk_repository_t* repository, sk_resource_object_t view, const sk_resource_field_t* field, sk_archive_writer_t* writer) {
	...
	case SK_RESOURCE_FIELD_TYPE_VEC2: {
		...
		write_f32_seq_named(writer, name, (const f32*)&v, 2u);
		return SK_RES_SER_OK;
	}

	case SK_RESOURCE_FIELD_TYPE_VEC3: {
		sk_vec3_t v = api->get_vec3(view, field->index);
		write_f32_seq_named(writer, name, (const f32*)&v, 3u);
		return SK_RES_SER_OK;
	}

	case SK_RESOURCE_FIELD_TYPE_VEC4: {
		sk_vec4_t v = api->get_vec4(view, field->index);
		write_f32_seq_named(writer, name, (const f32*)&v, 4u);
		return SK_RES_SER_OK;
	}
	...
	case SK_RESOURCE_FIELD_TYPE_MAT4: {
		...
		write_f32_seq_named(writer, name, v.m, 16u);
		return SK_RES_SER_OK;
	}
...
static i32 apply_field_value(sk_res_ser_resolve_ctx_t* ctx, sk_resource_object_t view, const sk_resource_field_t* field, sk_archive_reader_t* reader) {
	...
	case SK_RESOURCE_FIELD_TYPE_VEC2: {
		f64 comps[2] = {0.0, 0.0};
		if (read_f64_seq_named(reader, name, comps, 2u) == 0u) {
			return SK_RES_SER_OK; /* absent field: keep default */
		}
		sk_vec2_t v = {(f32)comps[0], (f32)comps[1]};
	...
	case SK_RESOURCE_FIELD_TYPE_VEC3: {
		f64 comps[3] = {0.0, 0.0, 0.0};
		if (read_f64_seq_named(reader, name, comps, 3u) == 0u) {
			return SK_RES_SER_OK; /* absent field: keep default */
		}
		sk_vec3_t v = {(f32)comps[0], (f32)comps[1], (f32)comps[2]};
	...
	case SK_RESOURCE_FIELD_TYPE_VEC4: {
		f64 comps[4] = {0.0, 0.0, 0.0, 0.0};
		if (read_f64_seq_named(reader, name, comps, 4u) == 0u) {
			return SK_RES_SER_OK; /* absent field: keep default */
		}
		sk_vec4_t v = {(f32)comps[0], (f32)comps[1], (f32)comps[2], (f32)comps[3]};
	...
	case SK_RESOURCE_FIELD_TYPE_QUAT: {
		...
		sk_quat_t v = {(f32)comps[0], (f32)comps[1], (f32)comps[2], (f32)comps[3]};
	...
	case SK_RESOURCE_FIELD_TYPE_MAT4: {
		f64 comps[16] = {0.0};
		if (read_f64_seq_named(reader, name, comps, 16u) == 0u) {
			return SK_RES_SER_OK; /* absent field: keep default */
		}
		sk_mat44_t v;
		for (u32 i = 0u; i < 16u; ++i) {
	...
	case SK_RESOURCE_FIELD_TYPE_COLOR: {
		...
		sk_color_t v = {(f32)comps[0], (f32)comps[1], (f32)comps[2], (f32)comps[3]};
...
static const sk_resource_type_t* ser_register_vec_enum(sk_repository_t* repo) {
	const sk_repository_api_t* api = sk_repository_api();
	sk_type_id_t tid;
...
SK_TEST(resource_serialize_vec_enum_roundtrip) {
	...
	sk_repository_t* repo = api->create(a);
	TEST_ASSERT_NOT_NULL(repo);
	const sk_resource_type_t* type = ser_register_vec_enum(repo);
	TEST_ASSERT_NOT_NULL(type);
	...
	/* serialize -> deserialize -> serialize produces identical JSON. */
	char* json_again = NULL;
	TEST_ASSERT_EQUAL_INT(0, sk_resource_serialize_json_alloc(repo, loaded, a, &json_again, NULL));
	TEST_ASSERT_EQUAL_STRING(json, json_again);
