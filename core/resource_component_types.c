...
SK_TEST(resource_component_types_register_and_resolve) {
	...
	TEST_ASSERT_EQUAL_INT(0, sk_resource_component_types_register(repo));
...
#undef TEST_COMPONENT_TYPE_REGISTERED
	...
	TEST_ASSERT_TRUE(SK_TYPE_ID_EQ(sk_resource_entity_component_type_id(repo, transform_rid), SK_TRANSFORM_COMPONENT_TYPE_ID));
