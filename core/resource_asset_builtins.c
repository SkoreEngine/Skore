...
SK_TEST(resource_asset_builtins_entity_resource_nested_roundtrip) {
	...
	TEST_ASSERT_TRUE(SK_TYPE_ID_EQ(sk_resource_entity_component_type_id(repo, comp_mesh), SK_MESH_RESOURCE_TYPE_ID));
	TEST_ASSERT_TRUE(SK_TYPE_ID_EQ(sk_resource_entity_component_type_id(repo, comp_mat), SK_MATERIAL_GRAPH_RESOURCE_TYPE_ID));
	TEST_ASSERT_TRUE(SK_TYPE_ID_EQ(sk_resource_entity_component_type_id(repo, SK_RID_ZERO), SK_TYPE_ID_ZERO));

	/* Round-trip the nested graph through the repository (package JSON). */
	char* json = NULL;
	TEST_ASSERT_EQUAL_INT(0, sk_resource_serialize_package_json_alloc(repo, parent, a, &json, NULL));
	...
	TEST_ASSERT_EQUAL_INT(0, sk_resource_deserialize_package_json_string(repo, sk_str_view_cstr(json), a, &loaded));
	...
	TEST_ASSERT_TRUE(SK_TYPE_ID_EQ(sk_resource_entity_component_type_id(repo, comps[0]), SK_MESH_RESOURCE_TYPE_ID));
	TEST_ASSERT_TRUE(SK_TYPE_ID_EQ(sk_resource_entity_component_type_id(repo, comps[1]), SK_MATERIAL_GRAPH_RESOURCE_TYPE_ID));
...
SK_TEST(resource_asset_builtins_scene_resource_roots_roundtrip) {
	...
	char* json = NULL;
	TEST_ASSERT_EQUAL_INT(0, sk_resource_serialize_package_json_alloc(repo, scene, a, &json, NULL));
	TEST_ASSERT_NOT_NULL(json);
	TEST_ASSERT_NOT_NULL(strstr(json, "sk.resource_package"));
	...
	TEST_ASSERT_TRUE(SK_TYPE_ID_EQ(api->type_id(api->resource_type(repo, roots[1])), SK_ENTITY_RESOURCE_TYPE_ID));
	{
		...
		TEST_ASSERT_NULL(api->get_subobject_list(rb, SK_ENTITY_RESOURCE_FIELD_CHILDREN, &b_child_count));
