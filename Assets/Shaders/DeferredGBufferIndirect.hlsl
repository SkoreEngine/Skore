...
PixelInput MainVS(uint vertexId : SV_VertexID, [[vk::builtin("BaseInstance")]] uint idx : SV_StartInstanceLocation)
{
	...
	output.uv1 = GetVertexUV1(vboff, layoutIdx, vertexId);

	float3x3 normalMat = (float3x3)instance.transform;
...
{
	MaterialSample material = SampleMaterial(input.matIndex, input.texCoord, input.normal, input.TBN);
