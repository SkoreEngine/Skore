                          ...
                          inout float3 intersection)
{
    ...
    if (intersected && abs(d.z) > 1e-6)
    {
        float  tHit    = (ray.z - o.z) / d.z;
        float2 baseSize = GetCellCount(0, texHiZ);
        float  texelT  = length(1.0 / baseSize) / max(1e-6, length(d.xy));
        float  tLo     = max(0.0, tHit - texelT * exp2((float)startLevel));
        float  tHi     = tHit;
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            float3 pMid   = o + d * tMid;
            float  sceneZ = LinearDepthToDeviceZ(texHiZ.SampleLevel(nearestSampler, pMid.xy, 0), nearClip, farClip);
            if (pMid.z >= sceneZ) tHi = tMid; else tLo = tMid;
        }
