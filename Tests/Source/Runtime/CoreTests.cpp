...
namespace
{
	...
	TEST_CASE("Core::JsonArchive::ReadWritePrimitivesAndNested")
	{
		JsonArchiveWriter writer;
		writer.WriteBool("enabled", true);
		writer.WriteInt("health", -42);
		writer.WriteUInt("score", 1234);
		writer.WriteFloat("ratio", 3.5);
		writer.WriteString("label", "player");

		writer.BeginMap("meta");
		writer.WriteString("name", "hero");
		writer.EndMap();

		writer.BeginSeq("values");
		writer.AddInt(10);
		writer.AddInt(20);
		writer.AddInt(30);
		writer.EndSeq();
		...
		JsonArchiveReader reader(json);
		CHECK(reader.ReadBool("enabled"));
		CHECK(reader.ReadInt("health") == -42);
		CHECK(reader.ReadUInt("score") == 1234);
		CHECK(reader.ReadFloat("ratio") == doctest::Approx(3.5));
		CHECK(reader.ReadString("label") == "player");

		REQUIRE(reader.BeginMap("meta"));
		CHECK(reader.ReadString("name") == "hero");
		reader.EndMap();

		REQUIRE(reader.BeginSeq("values"));
		i64 sum = 0;
		...
		REQUIRE(reader.BeginSeq("entries"));
		i64 idSum = 0;
		while (reader.NextSeqEntry())
		{
			reader.BeginMap();
			idSum += reader.ReadInt("id");
			reader.EndMap();
		}
		CHECK(idSum == 3);
		reader.EndSeq();
	...
	TEST_CASE("Core::JsonArchive::BlobAndMapIteration")
	{
		const u8 bytes[] = {5, 11, 99, 255};

		JsonArchiveWriter writer;
		writer.WriteUInt("value", 12);
		writer.WriteBlob("blob", bytes, sizeof(bytes));

		JsonArchiveReader reader(writer.EmitAsString());
		...
		{
			if (reader.GetCurrentKey() == "value")
			{
				CHECK(reader.GetUInt() == 12);
				foundValue = true;
			}
			else if (reader.GetCurrentKey() == "blob")
			{
				Span<u8> blob = reader.GetBlob();
				REQUIRE(blob.Size() == sizeof(bytes));
				for (usize i = 0; i < blob.Size(); ++i)
				{
					CHECK(blob[i] == bytes[i]);
				}
				foundBlob = true;
			}
		}
