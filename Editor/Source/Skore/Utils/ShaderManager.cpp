...
namespace Skore
{
	...
	void ShaderManagerInit()
	{
		...
		if (library)
		{
			dxcCreateInstance = reinterpret_cast<DxcCreateInstanceProc>(Platform::LoadFunction(library, "DxcCreateInstance"));
	...
	bool CompileShader(const ShaderCompileInfo& shaderCompileInfo, Array<u8>& bytes, String& log)
	{
		...
		{
			log += " DxShaderCompiler not loaded ";
			return false;
		}
		...
		if (shaderCompileInfo.api != GraphicsAPI::D3D12)
		{
			...
			bool isRayTracingStage = shaderCompileInfo.shaderStage == ShaderStage::RayGen
				|| shaderCompileInfo.shaderStage == ShaderStage::AnyHit
				|| shaderCompileInfo.shaderStage == ShaderStage::ClosestHit
				|| shaderCompileInfo.shaderStage == ShaderStage::Miss
				|| shaderCompileInfo.shaderStage == ShaderStage::Intersection
				|| shaderCompileInfo.shaderStage == ShaderStage::Callable;
			...
			{
				args.EmplaceBack(L"-fspv-target-env=vulkan1.1");
			}
	...
	namespace SpirvUtils
	{
		...
		void SortAndAddDescriptors(PipelineDesc& shaderInfo, const HashMap<u32, HashMap<u32, DescriptorSetLayoutBinding>>& descriptors)
		{
			...
			for (auto& set : sortDescriptors)
			{
				...
				{
					maxBinding = Max(maxBinding, bindingIt.second.binding);
				}
				descriptorLayout.bindings.Resize(maxBinding + 1);

				for (auto& it : bindings)
				{
					descriptorLayout.bindings[it.first] = it.second;
				}
