...
namespace Skore
{
	...
	template <usize T>
	struct ArchitectureDetect
	{
		static_assert(false, "Invalid architecture");
	};

	template <>
	struct ArchitectureDetect<4>
	{
		using Type = u32;
	};

	template <>
	struct ArchitectureDetect<8>
	{
		using Type = u64;
	};

	using uptr = ArchitectureDetect<sizeof(void*)>::Type;
...
template<typename  T>	\
T* ToPtr() { return static_cast<T*>(IntToPtr(handler)); }								       \
}
...
#define SK_RESOURCE_EXT ".resources"
#define SK_BUFFER_EXT ".buffers"
