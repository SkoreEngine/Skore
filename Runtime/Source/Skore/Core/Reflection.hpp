...
namespace Skore
{
	...
	public:
		...
		typedef void (*FnSetToPointer)(const ReflectValue* reflectValue, VoidPtr pointer);

		StringView GetDesc() const;
		i64        GetCode() const;
		ConstPtr   GetValue() const;
		bool       Compare(ConstPtr value) const;
		void       SetToPointer(VoidPtr pointer) const;
	...
	public:
		...
		{
			return HasAttribute(TypeInfo<T>::ID());
		}
	...
	{
	public:
		~ReflectFunction() override;
		typedef void (*FnInvoke)(const ReflectFunction* function, VoidPtr instance, VoidPtr ret, VoidPtr* params);

		ReflectFunction(StringView name) : name(name), simpleName(MakeSimpleName(name)) {}
		StringView          GetName() const;
	...
	public:
		...
		ReflectType(StringView name, StringView simpleName, TypeProps props);
		...
		Span<TypeID>             GetBaseTypes() const;
	...
	public:
		...
		void SetFnSetToPointer(ReflectValue::FnSetToPointer fnSetToPointer);
	...
	{
	public:
		static ReflectTypeBuilder RegisterType(StringView name, StringView simpleName, TypeProps props);
	...
	public:
		NativeReflectValue(ReflectValueBuilder builder)
		{
			...
			builder.SetFnSetToPointer(&FnSetToPointerImpl);
		}
	...
	private:
		...
		static void FnSetToPointerImpl(const ReflectValue* reflectValue, VoidPtr ptrValue)
		{
			*static_cast<Type*>(ptrValue) = *static_cast<const Type*>(&value);
		}
	};
	...
	public:
		...
		template <typename AttrType, typename... AttrArgs>
		decltype(auto) Attribute(AttrArgs&&... args)
		{
			NativeReflectAttribute<NativeReflectFunction, AttrType>::s_Value = AttrType{Traits::Forward<AttrArgs>(args)...,};
			NativeReflectAttribute<NativeReflectFunction, AttrType>(this->builder.AddAttribute(TypeInfo<AttrType>::GetProps()));
			return *this;
		}
	...
	private:
		static void InvokeImpl(const ReflectFunction* func, VoidPtr instance, VoidPtr ret, VoidPtr* params)
		{
			...
			else if constexpr (std::is_reference_v<Return> && !std::is_arithmetic_v<Traits::RemoveAll<Return>>)
			{
				auto& ref = fp(*static_cast<Traits::RemoveReference<Args>*>(params[--i])...);
				*static_cast<VoidPtr*>(ret) = const_cast<Traits::RemoveAll<Return>*>(std::addressof(ref));
			}
			else
			{
				new(PlaceHolder{}, ret) Traits::RemoveReference<Return>(fp(*static_cast<Traits::RemoveReference<Args>*>(params[--i])...));
			}
		}
	...
	public:
		...
		decltype(auto) Attribute(AttrArgs&&... args)
		{
			NativeReflectAttribute<NativeReflectMemberFunction, AttrType>::s_Value = AttrType{Traits::Forward<AttrArgs>(args)...,};
			NativeReflectAttribute<NativeReflectMemberFunction, AttrType>(this->builder.AddAttribute(TypeInfo<AttrType>::GetProps()));
			return *this;
		}
	...
	private:
		static void InvokeImpl(const ReflectFunction* func, VoidPtr instance, VoidPtr ret, VoidPtr* params)
		{
			...
			{
				auto& ref = (static_cast<Owner*>(instance)->*mfp)(*static_cast<Traits::RemoveReference<Args>*>(params[--i])...);
				*static_cast<VoidPtr*>(ret) = const_cast<Traits::RemoveAll<Return>*>(std::addressof(ref));
			}
			else
			{
				new(PlaceHolder{}, ret) std::remove_cv_t<std::remove_reference_t<Return>>((static_cast<Owner*>(instance)->*mfp)(*static_cast<Traits::RemoveReference<Args>*>(params[--i])...));
			}
		}
	...
	public:
		...
		{
			using DecompType = typename FunctionTemplateDecomposer<mfp, decltype(mfp)>::Type;
			static_assert(DecompType::params == sizeof...(TNames), "args and names should be equals");
			ReflectFunctionBuilder funcBuilder = builder.AddFunction(name);
            DecompType::AddParams(funcBuilder, Traits::Forward<TNames>(tnames)...);
			return DecompType(funcBuilder);
		}
	...
	{
		NativeReflectType<T> desc = NativeReflectType<T>(RegisterType(name, MakeSimpleName(name), TypeInfo<T>::GetProps()));
