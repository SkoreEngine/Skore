...
#define IMPL_HASH_32(_TYPE)\
	...
	{
		constexpr static bool hasHash = true;
		constexpr static usize Value(f64 v)
		{
			return AppendValue(v);
		}
	};
	...
	{
		constexpr static bool hasHash = true;
		constexpr static usize Value(const Pair<T, T>& v)
		{
			return Hash<T>::Value(v.first) ^ Hash<T>::Value(v.second);
		}
	};
	...
	struct Hash<T, std::enable_if_t<std::is_enum_v<T>>>
	{
		...
		constexpr static usize Value(const T& value)
		{
			using UnderlyingType = std::underlying_type_t<T>;
			return static_cast<usize>(static_cast<UnderlyingType>(value));
		}
	};
	...
	struct Hash<TypeID>
	{
		...
		constexpr static usize Value(const TypeID& typeId)
		{
			return Hash<u64>::Value(typeId.id);
		}
	};
	...
	constexpr void HashCombine(T& seed, T hash, Rest&&... rest)
	{
