...
namespace Skore
{
	...
	StringView ReflectFunction::GetSimpleName() const
	{
		return simpleName;
	}
	...
	void ReflectValue::SetToPointer(VoidPtr pointer) const
	{
		if (fnSetToPointer)
		{
			fnSetToPointer(this, pointer);
		}
	}
	...
	bool ReflectParam::HasAttribute(TypeID attributeId) const
	{
		return false;
	}
	...
	VoidPtr ReflectConstructor::GetUserData() const
	{
		return userData;
	}
	...
	Span<TypeID> ReflectType::GetBaseTypes() const
	{
		return baseTypes;
	}
	...
	void ReflectValueBuilder::SetFnSetToPointer(ReflectValue::FnSetToPointer fnSetToPointer)
	{
		reflectValue->fnSetToPointer = fnSetToPointer;
	}
	...
	void ReflectConstructorBuilder::SetUserData(VoidPtr userData)
	{
		constructor->userData = userData;
	}

	ReflectType::ReflectType(StringView name, StringView simpleName, TypeProps props) : name(name), props(props), simpleName(simpleName) {}
	...
	ReflectAttributeBuilder ReflectFunctionBuilder::AddAttribute(const TypeProps& props)
	{
		ReflectAttribute* reflectAttribute = Alloc<ReflectAttribute>(props);
		function->attributeArray.EmplaceBack(reflectAttribute);
	...
	ReflectTypeBuilder Reflection::RegisterType(StringView name, StringView simpleName, TypeProps props)
	{
		std::shared_ptr<ReflectType> reflectType = std::make_shared<ReflectType>(name, simpleName, props);
	...
	void ReflectionSetOptions(bool pReloadEnabled)
	{
		reloadEnabled = pReloadEnabled;
	}
