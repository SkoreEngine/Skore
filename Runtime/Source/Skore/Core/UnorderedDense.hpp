...
namespace Skore
{
    template <
        ...
        class KeyEqual = std::equal_to<Key>,
        class AllocatorOrContainer = StdAllocator<std::pair<Key, Value>>>
    using DenseMap = ankerl::unordered_dense::map<Key, Value, Hash, KeyEqual, AllocatorOrContainer>;

    template <
        class Key,
        class Hash = std::hash<Key>,
        class KeyEqual = std::equal_to<Key>,
        class AllocatorOrContainer = StdAllocator<Key>>
