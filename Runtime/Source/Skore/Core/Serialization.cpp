...
namespace Skore
{
	namespace
	{
		extern yyjson_alc alloc;

		enum class JsonIterType
		{
			Map,
			Seq
		};
		...
		struct JsonReaderState
		{
			...
			yyjson_val*  currentKey = nullptr;
			yyjson_val*  currentValue = nullptr;
			yyjson_obj_iter mapIter{};
			yyjson_arr_iter seqIter{};
			bool        iterInitialized = false;
			JsonIterType type = JsonIterType::Map;
		};

		struct JsonArchiveWriterContext
		{
			yyjson_mut_doc*        doc = nullptr;
			yyjson_mut_val*        current = nullptr;
			Array<yyjson_mut_val*> stack;

			JsonArchiveWriterContext() : stack(8)
			{
			...
			~JsonArchiveWriterContext()
			{
				yyjson_mut_doc_free(doc);
			}
		};

		struct JsonArchiveReaderContext
		{
			yyjson_doc*             doc = nullptr;
			Array<JsonReaderState>  stack;

			JsonArchiveReaderContext() : stack(8)
			{
			}

			~JsonArchiveReaderContext()
			{
				yyjson_doc_free(doc);
			}
		};

		yyjson_mut_val* JsonMakeKey(yyjson_mut_doc* doc, StringView key)
		{
			return yyjson_mut_strncpy(doc, key.Data(), key.Size());
		}

		void JsonWriteNamedValue(JsonArchiveWriterContext& context, StringView name, yyjson_mut_val* value)
		{
			SK_ASSERT(yyjson_mut_is_obj(context.current), "Current value must be an object for named writes");
		...
		yyjson_val* JsonGetCurrentValue(const JsonArchiveReaderContext& context)
		{
			...
			return context.stack.Back().currentValue;
		}

		yyjson_val* JsonFindNamedValue(const JsonArchiveReaderContext& context, StringView name)
		{
			if (context.stack.Empty() || name.Empty())
			{
				return nullptr;
			}

			yyjson_val* currentNode = context.stack.Back().node;
			if (!yyjson_is_obj(currentNode))
			{
				return nullptr;
			}

			return yyjson_obj_getn(currentNode, name.Data(), name.Size());
		}

		bool JsonReadBoolValue(yyjson_val* value)
		{
			return yyjson_get_bool(value);
		}

		i64 JsonReadIntValue(yyjson_val* value)
		{
			return static_cast<i64>(yyjson_get_sint(value));
		}

		u64 JsonReadUIntValue(yyjson_val* value)
		{
			return static_cast<u64>(yyjson_get_uint(value));
		}

		f64 JsonReadFloatValue(yyjson_val* value)
		{
			return static_cast<f64>(yyjson_get_num(value));
		}

		StringView JsonReadStringValue(yyjson_val* value)
		{
			...
			return StringView{yyjson_get_str(value), static_cast<usize>(yyjson_get_len(value))};
		}
	...
	{
		m_context = new JsonArchiveWriterContext();
	}

	JsonArchiveWriter::~JsonArchiveWriter()
	{
		delete static_cast<JsonArchiveWriterContext*>(m_context);
	}
	...
	void JsonArchiveWriter::WriteBool(StringView name, bool value)
	{
		...
		JsonWriteNamedValue(context, name, yyjson_mut_bool(context.doc, value));
	}

	void JsonArchiveWriter::WriteInt(StringView name, i64 value)
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		JsonWriteNamedValue(context, name, yyjson_mut_sint(context.doc, value));
	}

	void JsonArchiveWriter::WriteUInt(StringView name, u64 value)
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		JsonWriteNamedValue(context, name, yyjson_mut_uint(context.doc, value));
	}

	void JsonArchiveWriter::WriteFloat(StringView name, f64 value)
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		JsonWriteNamedValue(context, name, yyjson_mut_real(context.doc, value));
	}
	...
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		yyjson_mut_val* bytes = yyjson_mut_arr(context.doc);
		for (u64 index = 0; index < size; ++index)
		{
			yyjson_mut_arr_add_uint(context.doc, bytes, static_cast<const u8*>(data)[index]);
		}
		JsonWriteNamedValue(context, name, bytes);
	}

	void JsonArchiveWriter::AddBool(bool value)
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		JsonAddArrayValue(context, yyjson_mut_bool(context.doc, value));
	}

	void JsonArchiveWriter::AddInt(i64 value)
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		JsonAddArrayValue(context, yyjson_mut_sint(context.doc, value));
	}

	void JsonArchiveWriter::AddUInt(u64 value)
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		JsonAddArrayValue(context, yyjson_mut_uint(context.doc, value));
	}

	void JsonArchiveWriter::AddFloat(f64 value)
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		JsonAddArrayValue(context, yyjson_mut_real(context.doc, value));
	}
	...
	void JsonArchiveWriter::AddBlob(ConstPtr data, u64 size)
	{
		...
		JsonAddArrayValue(context, bytes);
	}

	void JsonArchiveWriter::BeginMap()
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		yyjson_mut_val* map = yyjson_mut_obj(context.doc);
		JsonAddArrayValue(context, map);
		context.stack.PushBack(context.current);
		context.current = map;
	}
	...
	void JsonArchiveWriter::BeginMap(StringView name)
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		yyjson_mut_val* map = yyjson_mut_obj(context.doc);
		JsonWriteNamedValue(context, name, map);
		context.stack.PushBack(context.current);
		context.current = map;
	}
	...
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		SK_ASSERT(!context.stack.Empty(), "No map scope to end");
		context.current = context.stack.Back();
	...
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		yyjson_mut_val* seq = yyjson_mut_arr(context.doc);
		JsonAddArrayValue(context, seq);
		context.stack.PushBack(context.current);
		context.current = seq;
	}
	...
	void JsonArchiveWriter::BeginSeq(StringView name)
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		yyjson_mut_val* seq = yyjson_mut_arr(context.doc);
		JsonWriteNamedValue(context, name, seq);
		context.stack.PushBack(context.current);
		context.current = seq;
	}
	...
	{
		JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		SK_ASSERT(!context.stack.Empty(), "No sequence scope to end");
		context.current = context.stack.Back();
		context.stack.PopBack();
	}
	...
	{
		const JsonArchiveWriterContext& context = *static_cast<JsonArchiveWriterContext*>(m_context);
		yyjson_write_err               err{};
		usize                          len = 0;
		char* json = yyjson_mut_val_write_opts(yyjson_mut_doc_get_root(context.doc), YYJSON_WRITE_ESCAPE_UNICODE | YYJSON_WRITE_PRETTY, &alloc, &len, &err);
		if (json == nullptr)
		{
			return {};
		}

		String output(json, len);
		Free(nullptr, json);
		return output;
	}

	JsonArchiveReader::JsonArchiveReader(StringView json)
	{
		m_context = new JsonArchiveReaderContext();
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		context.doc = yyjson_read(json.Data(), json.Size(), YYJSON_READ_NOFLAG);

		if (context.doc != nullptr)
		{
			yyjson_val* root = yyjson_doc_get_root(context.doc);
			if (yyjson_is_obj(root))
			{
				context.stack.PushBack(JsonReaderState{
					.node = root,
					.type = JsonIterType::Map
				});
			}
		}
	}
	...
	{
		delete static_cast<JsonArchiveReaderContext*>(m_context);
	}

	bool JsonArchiveReader::ReadBool(StringView name)
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadBoolValue(JsonFindNamedValue(context, name));
	}

	i64 JsonArchiveReader::ReadInt(StringView name)
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadIntValue(JsonFindNamedValue(context, name));
	}

	u64 JsonArchiveReader::ReadUInt(StringView name)
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadUIntValue(JsonFindNamedValue(context, name));
	}

	f64 JsonArchiveReader::ReadFloat(StringView name)
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadFloatValue(JsonFindNamedValue(context, name));
	}

	StringView JsonArchiveReader::ReadString(StringView name)
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadStringValue(JsonFindNamedValue(context, name));
	}

	bool JsonArchiveReader::GetBool()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadBoolValue(JsonGetCurrentValue(context));
	}

	i64 JsonArchiveReader::GetInt()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadIntValue(JsonGetCurrentValue(context));
	}

	u64 JsonArchiveReader::GetUInt()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadUIntValue(JsonGetCurrentValue(context));
	}

	f64 JsonArchiveReader::GetFloat()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadFloatValue(JsonGetCurrentValue(context));
	}

	StringView JsonArchiveReader::GetString()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		return JsonReadStringValue(JsonGetCurrentValue(context));
	}

	Span<u8> JsonArchiveReader::GetBlob()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		yyjson_val*             value = JsonGetCurrentValue(context);
		...
		m_blobBuffer.Clear();
		m_blobBuffer.Reserve(yyjson_arr_size(value));

		size_t idx, max;
		yyjson_val* entry;
		yyjson_arr_foreach(value, idx, max, entry)
		{
			m_blobBuffer.PushBack(static_cast<u8>(yyjson_get_uint(entry)));
		}

		return m_blobBuffer;
	}

	void JsonArchiveReader::BeginSeq()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		yyjson_val* node = JsonGetCurrentValue(context);
		SK_ASSERT(yyjson_is_arr(node), "Current value is not a sequence");

		context.stack.PushBack(JsonReaderState{
			.node = node,
			.type = JsonIterType::Seq
		});
	}

	bool JsonArchiveReader::BeginSeq(StringView name)
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		yyjson_val* node = JsonFindNamedValue(context, name);
	...
	bool JsonArchiveReader::NextSeqEntry()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		if (context.stack.Empty())
		{
		...
		JsonReaderState& state = context.stack.Back();
		if (state.type != JsonIterType::Seq)
		{
			return false;
		}

		if (!state.iterInitialized)
		{
			yyjson_arr_iter_init(state.node, &state.seqIter);
			state.iterInitialized = true;
		}

		state.currentValue = yyjson_arr_iter_next(&state.seqIter);
		return state.currentValue != nullptr;
	}

	void JsonArchiveReader::EndSeq()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		SK_ASSERT(!context.stack.Empty(), "No sequence scope to end");
		context.stack.PopBack();
	...
	void JsonArchiveReader::BeginMap()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		yyjson_val* node = JsonGetCurrentValue(context);
		SK_ASSERT(yyjson_is_obj(node), "Current value is not a map");
	...
	bool JsonArchiveReader::BeginMap(StringView name)
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		yyjson_val* node = JsonFindNamedValue(context, name);
	...
	bool JsonArchiveReader::NextMapEntry()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		if (context.stack.Empty())
		{
		...
		JsonReaderState& state = context.stack.Back();
		if (state.type != JsonIterType::Map)
		{
			return false;
		}
		...
		if (!state.iterInitialized)
		{
			yyjson_obj_iter_init(state.node, &state.mapIter);
			state.iterInitialized = true;
		}

		state.currentKey = yyjson_obj_iter_next(&state.mapIter);
		if (state.currentKey == nullptr)
		{
			state.currentValue = nullptr;
			return false;
		}

		state.currentValue = yyjson_obj_iter_get_val(state.currentKey);
		return state.currentValue != nullptr;
	}

	StringView JsonArchiveReader::GetCurrentKey()
	{
		JsonArchiveReaderContext& context = *static_cast<JsonArchiveReaderContext*>(m_context);
		if (context.stack.Empty())
		{
		...
		return JsonReadStringValue(state.currentKey);
	}
