...
namespace Skore
{
	...
	void RegisterCursorLockModeEnum()
	{
		auto cursorLockModeEnum = Reflection::Type<CursorLockMode>();

		cursorLockModeEnum.Value<CursorLockMode::Free>("Free");
		cursorLockModeEnum.Value<CursorLockMode::Locked>("Locked");
	}
	...
	void RegisterInputType()
	{
		auto inputType = Reflection::Type<Input>();

		inputType.Function<&Input::IsKeyDown>("IsKeyDown", "key");
		inputType.Function<&Input::IsKeyPressed>("IsKeyPressed", "key");
		inputType.Function<&Input::IsKeyReleased>("IsKeyReleased", "key");
		inputType.Function<&Input::IsMouseDown>("IsMouseDown", "mouseButton");
		inputType.Function<&Input::IsAnyMouseDown>("IsAnyMouseDown");
		inputType.Function<&Input::IsMouseClicked>("IsMouseClicked", "mouseButton");
		inputType.Function<&Input::IsMouseReleased>("IsMouseReleased", "mouseButton");
		inputType.Function<&Input::GetMousePosition>("GetMousePosition");
		inputType.Function<&Input::GetMouseWheel>("GetMouseWheel");
		inputType.Function<&Input::IsMouseMoving>("IsMouseMoving");
		inputType.Function<&Input::SetCursorLockMode>("SetCursorLockMode", "cursorLockMode");
		inputType.Function<&Input::GetCursorLockMode>("GetCursorLockMode");
		inputType.Function<&Input::GetMouseAxis>("GetMouseAxis");
		inputType.Function<&Input::DisableInputs>("DisableInputs", "disable");
	}
	...
	void RegisterIOTypes()
	{
		...
		RegisterCursorLockModeEnum();
		RegisterAccessModeEnum();

		// Register structs
		RegisterStructTypes();

		// Register Input type with static functions
		RegisterInputType();
