...
namespace Skore
{
	struct ForwardPass : DefaultPipelinePass
	{
		SK_CLASS(ForwardPass, DefaultPipelinePass);
		...
		void BuildRenderGraph(RenderGraph& renderGraph) override
		{
			...
			renderGraph
				...
				.Render([this](RenderGraphPass& pass, Scene* scene, GPUCommandBuffer* cmd)
				{
					...
					SceneCullingData* cullingData = rg.GetInstanceData<SceneCullingData>(SceneCullingDataName);
		...
		{
			RID forwardShader = Resources::FindByPath("Skore://Shaders/DefaultForward.shader");
		...
		void RenderBucket(GPUCommandBuffer* cmd, RenderGraph& rg, PipelineBucket& bucket, Array<DrawPipeline>& drawPipelines, GPUDescriptorSet* sceneSet, GPUDescriptorSet* globalSet, GPUDescriptorSet* skinningSet, SceneCullingData* cullingData)
		{
