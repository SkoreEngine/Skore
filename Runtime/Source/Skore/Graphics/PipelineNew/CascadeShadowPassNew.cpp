...
namespace Skore
{
	namespace
	{
		...
		constexpr u32 kMaxShadowPipelinesNew = MaxShadowCullSlotsNew / MaxNumCascades;
	...
	struct CascadeShadowPassNew : DefaultPipelinePassNew
	{
		SK_CLASS(CascadeShadowPassNew, DefaultPipelinePassNew);
		...
		u32              slotOffsets[MaxShadowCullSlotsNew] = {};
		u32              slotMaxDraws[MaxShadowCullSlotsNew] = {};
		u32              pipesThisFrame = 0;

		bool instanceDataInitialized = false;

		~CascadeShadowPassNew() override
		{
		...
		void EnsureShadowPipelines(RenderSceneObjects* objects, GPURenderPass* renderPass, GPUDescriptorSet* pipelineSceneSet, GPUDescriptorSet* globalSet, GPUDescriptorSet* skinningSet)
		{
			...
			u32 count = Math::Min(static_cast<u32>(objects->shadowPipelines.Size()), kMaxShadowPipelinesNew);
		...
		{
			ShadowMapInstanceDataNew* shadowData = renderGraph.CreateInstance<ShadowMapInstanceDataNew>(ShadowMapInstanceDataNewName);
			if (!instanceDataInitialized)
			{
				*shadowData = ShadowMapInstanceDataNew{};
			...
			const u32 numShadowPipes = Math::Min(static_cast<u32>(objects->shadowPipelines.Size()), kMaxShadowPipelinesNew);
			...
			renderGraph.Create(ShadowCountsName, RenderGraphBufferDesc{
				.size = sizeof(u32) * MaxShadowCullSlotsNew,
				.usage = ResourceUsage::IndirectBuffer,
				.hostVisible = false,
				.perFrame = true
			});
			...
			renderGraph
				...
				.Render([this](RenderGraphPass& pass, Scene* scene, GPUCommandBuffer* cmd)
				{
					...
					memcpy(offsets->GetMappedData(), slotOffsets, sizeof(u32) * MaxShadowCullSlotsNew);
					memcpy(cullData->GetMappedData(), &cullDataCB, sizeof(ShadowCullDataCB));
					cmd->FillBuffer(counts, 0, sizeof(u32) * MaxShadowCullSlotsNew, 0);
				});

			renderGraph
				.AddComputePass("ShadowCulling", "Skore://ShadersNew/CullingShadowPassNew.comp")
				.Stage(RenderStage::ShadowPass)
				.Write(drawsName)
	...
	{
		Reflection::Type<CascadeShadowPassNew>();
	}
}
