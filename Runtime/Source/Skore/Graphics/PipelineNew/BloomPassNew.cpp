...
namespace Skore
{
	struct BloomPassNew : DefaultPipelinePassNew
	{
		SK_CLASS(BloomPassNew, DefaultPipelinePassNew);
		...
		void BuildRenderGraph(RenderGraph& renderGraph) override
		{
			...
			u32 mipCount = BloomMipCountNew(outputSize);
			...
			renderGraph.Create(BloomTextureNewName, RenderGraphTextureDesc{
...

			{
				...
				renderGraph
					.AddComputePass("BloomPrefilter", "Skore://ShadersNew/BloomPrefilterNew.comp")
			...
			for (u32 m = 1; m < mipCount; ++m)
			{
				...
				renderGraph
					.AddComputePass(String("BloomDownsample_") + ToString(m), "Skore://ShadersNew/BloomDownsampleNew.comp")
					.Stage(RenderStage::Composite)
					.Read(mipNames[m - 1])
					.Write(mipNames[m])
					.Constants<BloomPushConstants>([pc](RenderGraph&, BloomPushConstants& constants)
			...
			for (i32 m = static_cast<i32>(mipCount) - 2; m >= 0; --m)
			{
				...
				renderGraph
					.AddComputePass(String("BloomUpsample_") + ToString(dstMip), "Skore://ShadersNew/BloomUpsampleNew.comp")
					.Stage(RenderStage::Composite)
