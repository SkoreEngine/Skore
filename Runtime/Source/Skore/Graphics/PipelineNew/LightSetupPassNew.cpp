...
namespace Skore
{
	//Fills the per-frame light UBO from LightComponent/EnvironmentComponent and points the scene
	//descriptor set (space1) at the light buffer, shadow map + comparison sampler (bindings 3/4) and
	//environment IBL textures consumed by the forward shader (LightsNew.hlsli).
	//Descriptor updates happen at BUILD time: other passes bind the scene set earlier in the command
	//buffer, and updating a bound set mid-recording invalidates the whole command buffer. Only the
	//mapped light-buffer write stays at record time so it sees same-frame cascade data.
	struct LightSetupPassNew : DefaultPipelinePassNew
	{
		SK_CLASS(LightSetupPassNew, DefaultPipelinePassNew);
		...
		Array<LightDataNew> lightData;
		u32                 shadowLightIndex = U32_MAX;

		~LightSetupPassNew() override
		{
		...
		void EnsureResources()
		{
			...
			lightBufferAlignedSize = AlignedSize(static_cast<u64>(sizeof(LightBufferNew)), uboAlignment);
		...
		void EnsureLightStorage(u32 lightCount)
		{
			...
			lightStorageBuffer = Graphics::CreateBuffer(BufferDesc{
				.size = sizeof(LightDataNew) * lightStorageCapacity,
				.usage = ResourceUsage::ShaderResource,
				.hostVisible = true,
				.persistentMapped = true,
				.debugName = "LightStorageBufferNew"
			});
		}
		...
		void BuildRenderGraph(RenderGraph& renderGraph) override
		{
			...
			LightInstanceDataNew* instanceData = renderGraph.CreateInstance<LightInstanceDataNew>(LightInstanceDataNewName);
			...
			instanceData->flags = LightFlagsNew::None;
					...
					switch (env->GetAmbientLightSource())
					{
						case AmbientLightSource::Skybox:
							if (environmentCache && environmentCache->diffuseIrradianceTexture && environmentCache->cubeMapSkyTexture)
							{
								...
								instanceData->flags |= LightFlagsNew::HasAmbientTexture;
							}
							break;
						case AmbientLightSource::Color:
							instanceData->ambientMultiplier = env->GetAmbientLightIntensity();
							instanceData->ambientLight = env->GetAmbientLightColor().ToVec3();
							instanceData->flags |= LightFlagsNew::HasAmbientColor;
							break;
					...
					switch (env->GetReflectedLightSource())
					{
						case ReflectedLightSource::Skybox:
							if (environmentCache && environmentCache->specularMapTexture)
							{
								...
								instanceData->flags |= LightFlagsNew::HasReflectionTexture;
							}
							break;
			...
			lightUpdate.bufferRange = sizeof(LightBufferNew);
			...
			lightStorageUpdate.bufferRange = sizeof(LightDataNew) * lightStorageCapacity;
			sceneSet->Update(lightStorageUpdate);

			ShadowMapInstanceDataNew* shadowData = renderGraph.GetInstanceData<ShadowMapInstanceDataNew>(ShadowMapInstanceDataNewName);
			...
			{
				//the forward shader statically references the shadow bindings; without a shadow map
				//this frame (e.g. very first frame) drawing would consume invalid descriptors
				return;
			}
			...
			renderGraph
				...
				.Write(LightInstanceDataNewName)
				.Render([this](RenderGraphPass& pass, Scene* scene, GPUCommandBuffer* cmd)
				{
					RenderGraph& rg = *pass.GetGraph();

					LightInstanceDataNew* instanceData = rg.GetInstanceData<LightInstanceDataNew>(LightInstanceDataNewName);
					if (instanceData == nullptr) return;

					LightBufferNew lightBufferData{};
					...
					ShadowMapInstanceDataNew* shadowData = rg.GetInstanceData<ShadowMapInstanceDataNew>(ShadowMapInstanceDataNewName);
					...
					new(lightMem) LightBufferNew(lightBufferData);

					if (lightStorageBuffer != nullptr && !lightData.Empty())
					{
						memcpy(lightStorageBuffer->GetMappedData(), lightData.Data(), sizeof(LightDataNew) * lightData.Size());
					}
				});
		}
	};
	...
	{
		Reflection::Type<LightSetupPassNew>();
	}
}
