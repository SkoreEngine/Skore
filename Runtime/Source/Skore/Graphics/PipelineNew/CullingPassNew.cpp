...
namespace Skore
{
	...
	struct CullingPassNew : DefaultPipelinePassNew
	{
		SK_CLASS(CullingPassNew, DefaultPipelinePassNew);
		...
		{
			SceneCullingDataNew* data = renderGraph.CreateInstance<SceneCullingDataNew>(SceneCullingDataNewName);
			...
			u32 pipelineCount = Math::Min(static_cast<u32>(objects->opaquePipelines.Size()), MaxCullingPipelinesNew);
			...
			for (u32 i = pipelineCount; i < MaxCullingPipelinesNew; ++i)
			{
			...
			renderGraph.Create(OpaqueCountsName, RenderGraphBufferDesc{
				.size = sizeof(u32) * MaxCullingPipelinesNew,
				.usage = ResourceUsage::IndirectBuffer,
				.hostVisible = false,
				.perFrame = true
			});
			...
			renderGraph
				...
				.Render([this](RenderGraphPass& pass, Scene* scene, GPUCommandBuffer* cmd)
				{
					...
					SceneCullingDataNew* data = rg.GetInstanceData<SceneCullingDataNew>(SceneCullingDataNewName);
					...
					memcpy(offsets->GetMappedData(), data->drawOffsets, sizeof(u32) * MaxCullingPipelinesNew);
					cmd->FillBuffer(counts, 0, sizeof(u32) * MaxCullingPipelinesNew, 0);
			...
			renderGraph
				.AddComputePass("Culling", "Skore://ShadersNew/CullingPassNew.comp")
				.Stage(RenderStage::Culling)
				.Write(drawsName)
	...
	{
		Reflection::Type<CullingPassNew>();
	}
}
