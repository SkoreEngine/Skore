...
namespace Skore
{
	...
	VkBufferUsageFlags GetBufferUsageFlags(ResourceUsage usage, bool supportBufferDeviceAddress)
	{
		...
		if ((static_cast<u32>(usage) & static_cast<u32>(ResourceUsage::AccelerationStructure)) != 0)
	...
	void CreateDescriptorSetLayout(VkDevice vkDevice, Span<DescriptorSetLayoutBinding> bindings, VkDescriptorSetLayout* descriptorSetLayout, bool* hasRuntimeArrays, bool pushDescriptor)
	{
		...
		{
			if (binding.descriptorType == DescriptorType::None) continue;

			vkBindings.EmplaceBack(VkDescriptorSetLayoutBinding{
				.binding = binding.binding,
				.descriptorType = ConvertDescriptorType(binding.descriptorType),
				.descriptorCount = binding.renderType != RenderType::RuntimeArray ? binding.count : MaxBindlessResources,
	                              ...
	                              Array<VkDescriptorSetLayout>& descriptorSetLayouts, bool pushDescriptor, Span<DescriptorSetOverride> overrides)
	{
		...
		return vkCreatePipelineLayout(vkDevice, &layoutCreateInfo, nullptr, vkPipelineLayout);
	}
	...
	VkFormat ToVkFormat(Format format)
	{
		switch (format)
		{
			...
			case Format::RGB16_FLOAT: return VK_FORMAT_R16G16B16_SFLOAT;
			...
			case Format::ETC1_UNORM: return VK_FORMAT_ETC2_R8G8B8_UNORM_BLOCK; // ETC1 is supported through ETC2 in Vulkan
			case Format::ETC2_UNORM: return VK_FORMAT_ETC2_R8G8B8_UNORM_BLOCK;
			case Format::ETC2_SRGB: return VK_FORMAT_ETC2_R8G8B8_SRGB_BLOCK;
			case Format::ETC2A_UNORM: return VK_FORMAT_ETC2_R8G8B8A8_UNORM_BLOCK;
			case Format::ETC2A_SRGB: return VK_FORMAT_ETC2_R8G8B8A8_SRGB_BLOCK;
	...
	Format ToFormat(VkFormat format)
	{
		switch (format)
		{
			...
			case VK_FORMAT_B8G8R8A8_SNORM:
				return Format::BGRA8_SNORM;
			case VK_FORMAT_B8G8R8A8_UINT:
				return Format::BGRA8_UINT;
			case VK_FORMAT_B8G8R8A8_SINT:
				return Format::BGRA8_SINT;
			...
			case VK_FORMAT_ETC2_R8G8B8_SRGB_BLOCK:
				return Format::ETC2_SRGB;
			case VK_FORMAT_ETC2_R8G8B8A8_UNORM_BLOCK:
				return Format::ETC2A_UNORM;
			case VK_FORMAT_ETC2_R8G8B8A8_SRGB_BLOCK:
				return Format::ETC2A_SRGB;
	...
	VkBuildAccelerationStructureFlagsKHR ConvertBuildASFlags(BuildAccelerationStructureFlags flags)
	{
		...
		u32 f = static_cast<u32>(flags);

		if (f & static_cast<u32>(BuildAccelerationStructureFlags::AllowUpdate))
			result |= VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_UPDATE_BIT_KHR;
		if (f & static_cast<u32>(BuildAccelerationStructureFlags::AllowCompaction))
			result |= VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_KHR;
		if (f & static_cast<u32>(BuildAccelerationStructureFlags::PreferFastTrace))
			result |= VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR;
		if (f & static_cast<u32>(BuildAccelerationStructureFlags::PreferFastBuild))
			result |= VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_BUILD_BIT_KHR;
		if (f & static_cast<u32>(BuildAccelerationStructureFlags::MinimizeMemory))
