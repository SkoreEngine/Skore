...
namespace Skore
{
	...
	public:
		...
		ResourceObject& operator=(ResourceObject&& resourceObject) noexcept;
		~ResourceObject();
		...
		void SetTypeID(u32 index, TypeID value);
		...
		ResourceBuffer GetBuffer(u32 index) const;
		...
		void IterateSubObjects(FnSubObjectCallback callback, VoidPtr userData) const;
		void IterateAllSubObjects(FnSubObjectCallback callback, VoidPtr userData) const;
		void IterateAllBuffers(FnResourceBufferCallback callback, VoidPtr userData) const;
		...
		template<typename T>
		void IterateAllBuffers(T&& func) const
		{
			IterateAllBuffers([](const ResourceBuffer& buffer, VoidPtr userData)
			{
				(*static_cast<Traits::RemoveAll<T>*>(userData))(buffer);
			}, &func);
		}

		template<typename T>
		void IterateSubObjects(T&& func) const
		{
			IterateSubObjects([](u32 index, RID rid, VoidPtr userData)
			{
				(*static_cast<Traits::RemoveAll<T>*>(userData))(index, rid);
			}, &func);
		}

		template <typename T>
		void IterateAllSubObjects(T&& func) const
		{
			IterateAllSubObjects([](u32 index, RID rid, VoidPtr userData)
			{
				(*static_cast<Traits::RemoveAll<T>*>(userData))(index, rid);
			}, &func);
		}
