...
#define RESOURCE_CAST(TYPE, TYPE_NAME)   \
	...
	RESOURCE_CAST(Mat4, Mat4);

	//other
	RESOURCE_CAST(Color, Color);

	//TypeID
	RESOURCE_CAST(TypeID, TypeID);
	...
	template<typename T>
	struct ResourceCast<SubObjectRID<T>>
	{
		constexpr static bool hasSpecialization = true;

		static void ToResource(ResourceObject& object, u32 index, UndoRedoScope* scope, const SubObjectRID<T>& value, VoidPtr userData)
		{
			object.SetSubObject(index, value);
		}

		static void FromResource(const ResourceObject& object, u32 index, SubObjectRID<T>& value, VoidPtr userData)
		{
			value.id = object.GetSubObject(index).id;
		}
