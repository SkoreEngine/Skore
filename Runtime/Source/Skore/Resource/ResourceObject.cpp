...
namespace Skore
{
	...
	ResourceObject::ResourceObject() : m_storage(nullptr), m_currentInstance(nullptr) {}
	...
	ResourceObject& ResourceObject::operator=(ResourceObject&& resourceObject) noexcept
	{
	...
	void ResourceObject::SetMat4(u32 index, const Mat4& value)
	{
		SK_ASSERT(m_storage->resourceType->fields[index]->type == ResourceFieldType::Mat4, "Invalid field type");
		SetValue(index, &value, sizeof(Mat4));
	}
	...
	void ResourceObject::SetBuffer(u32 index, ResourceBuffer buffer)
	{
		SK_ASSERT(m_storage->resourceType->fields[index]->type == ResourceFieldType::Buffer, "Invalid field type");
		if (ResourceBuffer* bufferPtr = GetMutPtr<ResourceBuffer>(index))
		{
			*bufferPtr = buffer;
		}
	}

	void ResourceObject::SetTypeID(u32 index, TypeID value)
	{
		SK_ASSERT(m_storage->resourceType->fields[index]->type == ResourceFieldType::TypeID, "Invalid field type");
		SetValue(index, &value, sizeof(TypeID));
	}
	...
	{
		usize fieldSize = m_storage->resourceType->GetFields()[index]->GetTypeStaticProps().size;
		if (fieldSize > size)
		{
			return false;
		}
	...
	Mat4 ResourceObject::GetMat4(u32 index) const
	{
		if (const Mat4* value = GetPtr<Mat4>(index))
		{
			return *value;
		}
		return {};
	...
	TypeID ResourceObject::GetTypeID(u32 index) const
	{
		if (const TypeID* value = GetPtr<TypeID>(index))
		{
			return *value;
		}
		return {};
	...
	ResourceBuffer ResourceObject::GetBuffer(u32 index) const
	{
		if (const ResourceBuffer* value = GetPtr<ResourceBuffer>(index))
		{
			return *value;
		}
		return {};
	...
	void ResourceObject::IterateSubObjects(FnSubObjectCallback callback, VoidPtr userData) const
	{
		for (ResourceField* field : m_storage->resourceType->GetFields())
		{
			if (field && HasValueOnThisObject(field->GetIndex()))
			{
				switch (field->GetType())
				{
					case ResourceFieldType::SubObject:
						callback(field->GetIndex(), GetSubObject(field->GetIndex()), userData);
						break;
					case ResourceFieldType::SubObjectList:
						IterateSubObjectList(field->GetIndex(), [&](RID rid)
						{
							callback(field->GetIndex(), rid, userData);
						});
						break;
					default:
						break;
	...
	void ResourceObject::IterateAllSubObjects(FnSubObjectCallback callback, VoidPtr userData) const
	{
		RID             currentRID = GetRID();
		std::queue<RID> pending;

		while (currentRID)
		{
			if (ResourceObject object = Resources::Read(currentRID))
			{
				object.IterateSubObjects([&](u32 index, RID rid)
				{
					callback(index, rid, userData);
					pending.push(rid);
				});
			}
			...
			if (!pending.empty())
			{
				currentRID = pending.front();
				pending.pop();
			}
		}
	}

	void ResourceObject::IterateAllBuffers(FnResourceBufferCallback callback, VoidPtr userData) const
	{
		auto iterateBufferStorage = [&](const ResourceObject& object)
		{
			if (object && object.GetType())
			{
				for (ResourceField* field : object.GetType()->GetFields())
				{
					if (field->GetType() == ResourceFieldType::Buffer)
					{
						if (object.HasValueOnThisObject(field->GetIndex()))
						{
							callback(object.GetBuffer(field->GetIndex()), userData);
						}
					}
				}
			}
		};
		iterateBufferStorage(*this);
		IterateAllSubObjects([&](u32 index, RID rid)
		{
			iterateBufferStorage(Resources::Read(rid));
		});
	}
	...
	ResourceInstance ResourceObject::GetInstance() const
	{
		return m_currentInstance ? m_currentInstance : m_storage->instance.load();
	}
	...
	ConstPtr ResourceObject::GetPtr(u32 index) const
	{
		...
		if (m_storage->resourceType->fields.Size() <= index) return nullptr;
