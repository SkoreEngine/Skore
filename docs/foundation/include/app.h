...
typedef struct sk_app_api_t {
	 ...
	 * Does **not** call `sk_logger_bind_api` (that symbol is deleted).
	 *
	 ...
	 * registry-only context that never ran startup. `sk_platform_init` is deleted.
	 *
	 * @param context App context (must not be NULL).
 ...
 * (unmaps leftover views), frees registries, and (if this context installed
 * it) drops one crash-handler refcount. Caller must not use @p context after.
 *
