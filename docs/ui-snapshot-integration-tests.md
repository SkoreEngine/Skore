...
| `ui_text_screenshot_suite_completeness` | `ui_text_screenshot.c` | Yes (full suite) | `{root}/text-screenshot/msdf/*` |
...
Standalone entry: `sk-text-screenshot [--verify]` (MSDF pipeline only since
APX-271). Writes a complete capture set under
`{SK_TEST_ARTIFACT_DIR}/text-screenshot/msdf/`:

- `pangram.png`, `sizes.png`, `colors_alpha.png`, `scaled.png`, `glyph_grid.png`
- `atlas_msdf.png` (raw RGB8 MSDF atlas dump)
- `manifest.txt` (name + byte size)

`--verify` re-captures each sample and byte-compares raw readback + PNG
(and the atlas dump on `glyph_grid`). CTest: `sk-text-screenshot`. Rotation is
not supported by the UI (no transform API); the suite covers 2× content-scale
instead.
