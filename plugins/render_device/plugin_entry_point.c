...
/* Defined in render_device.c; registers the static API table on the app. */
 ...
 * via sk_render_device_init. Hosts look it up with app_api->get_api.
 *
...
SK_API int sk_plugin_entry_point(sk_app_context_t* context, const sk_app_api_t* app_api) {
	sk_render_device_init(context, app_api);
	return 0;
}
