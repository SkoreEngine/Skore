...
/* Defined in test_render_device.c; registers the stub API table on the app. */
 ...
 *
 * Registers sk_render_device_api_t under SK_RENDER_DEVICE_API_TYPE_ID
 * via sk_test_render_device_init (test double with stub functions).
 * Hosts look it up with app_api->get_api.
 *
...
SK_API int sk_plugin_entry_point(sk_app_context_t* context, const sk_app_api_t* app_api) {
	sk_test_render_device_init(context, app_api);
	return 0;
}
