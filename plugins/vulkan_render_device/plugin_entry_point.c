...
/* Defined in vulkan_render_device.c; registers the static API table on the app. */
 ...
 *
 * Registers sk_vulkan_render_device_api_t under SK_VULKAN_RENDER_DEVICE_API_TYPE_ID
 * via sk_vulkan_render_device_init. Hosts look it up with app_api->get_api.
 *
...
SK_API int sk_plugin_entry_point(sk_app_context_t* context, const sk_app_api_t* app_api) {
	sk_vulkan_render_device_init(context, app_api);
	return 0;
}
