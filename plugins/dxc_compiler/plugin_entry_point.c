 ...
 *
 * Registers sk_dxc_compiler_api_t under SK_DXC_COMPILER_API_TYPE_ID via
 * sk_dxc_compiler_init. Hosts look it up with app_api->get_api.
 *
...
SK_API int sk_plugin_entry_point(sk_app_context_t* context, const sk_app_api_t* app_api) {
	sk_dxc_compiler_init(context, app_api);
	return 0;
}
