...
/* Defined in entities.c; registers the static API table on the app. */
 ...
 *
 * Registers sk_entities_api_t under SK_ENTITIES_API_TYPE_ID via
 * sk_entities_init. Hosts look it up with app_api->get_api.
 *
...
SK_API int sk_plugin_entry_point(sk_app_context_t* context, const sk_app_api_t* app_api) {
	sk_entities_init(context, app_api);
	return 0;
}
