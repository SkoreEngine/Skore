 ...
 * @brief Foundation-internal table install hooks.
 *
...
/**
 * Write `ctx->logger_api` only. Defined in `logger.c`.
 * Does not create a logger context. Does not call `set_api`.
 */
void sk_logger_install(sk_app_context_t* ctx);

/**
 * Write `ctx->filesystem_api` only.
 * Defined in `filesystem_unix.c` / `filesystem_win32.c`.
 * Does not create a filesystem context. Does not call `set_api`.
 */
void sk_filesystem_install(sk_app_context_t* ctx);

/**
 * Write `ctx->platform` only.
...
/**
 * Write `ctx->repository_api` only. Defined in `repository.c`.
 * Does not call `set_api`.
 */
void sk_repository_install(sk_app_context_t* ctx);

/**
 * Write `ctx->resource_assets_api` only. Defined in `resource_assets.c`.
 * Does not call `set_api`.
 */
