...
i32 sk_jolt_component_types_register(sk_repository_t* repository, const sk_repository_api_t* repo_api) {
	for (u32 i = 0u; i < jolt_component_type_desc_count; ++i) {
		i32 rc = repo_api->register_type(repository, jolt_component_type_descs[i]);
		if (rc != 0) {
			return rc;
		}
	}
...
SK_TEST(jolt_component_types_register_and_list) {
	/* Repository function table is process-lifetime static data; fetch it from
	 * a registry-only context like the other foundation tests. */
