/**
 * @file test_cases.c
 * @brief Host-only SK_TESTs for header-only modules and the test harness.
 *
 * Linked only into sk-foundation-tests (the sk-tests host). Not part of
 * sk-test.a — plugins statically link that registry and must not re-run
 * these cases on every plugin scan.
 */
...
/* ---- harness filter / CLI / integration gate ---- */

SK_TEST(test_filter_empty_matches_all) {
	TEST_ASSERT_EQUAL_INT(1, sk_test_name_matches_filter("anything", NULL));
...
SK_TEST(test_parse_cli_filter_list_and_plugins_dir) {
	char arg0[] = "sk-tests";
	char arg1[] = "--list";
	char arg2[] = "--filter=ui_dock_*";
	char arg3[] = "C:/plugins";
	char* argv[] = {arg0, arg1, arg2, arg3, NULL};
	sk_test_cli_t cli;

	TEST_ASSERT_EQUAL_INT(0, sk_test_parse_cli(4, argv, &cli));
	TEST_ASSERT_EQUAL_INT(1, cli.list_only);
	TEST_ASSERT_EQUAL_INT(0, cli.help);
	TEST_ASSERT_EQUAL_STRING("ui_dock_*", cli.filter);
	TEST_ASSERT_EQUAL_STRING("C:/plugins", cli.plugins_dir);
	TEST_ASSERT_NULL(cli.unknown_opt);
}

SK_TEST(test_parse_cli_filter_separate_arg_and_help) {
	char arg0[] = "sk-integration-tests";
	char arg1[] = "--filter";
	char arg2[] = "vulkan_*";
	char arg3[] = "-h";
	char* argv[] = {arg0, arg1, arg2, arg3, NULL};
	...
	TEST_ASSERT_EQUAL_INT(0, sk_test_parse_cli(4, argv, &cli));
	TEST_ASSERT_EQUAL_INT(1, cli.help);
	TEST_ASSERT_EQUAL_STRING("vulkan_*", cli.filter);
	TEST_ASSERT_NULL(cli.plugins_dir);
}

SK_TEST(test_parse_cli_unknown_flag_fails) {
	char arg0[] = "sk-tests";
	char arg1[] = "--nope";
	char* argv[] = {arg0, arg1, NULL};
	sk_test_cli_t cli;

	TEST_ASSERT_NOT_EQUAL_INT(0, sk_test_parse_cli(2, argv, &cli));
	TEST_ASSERT_EQUAL_STRING("--nope", cli.unknown_opt);
}

SK_TEST(test_integration_gate_direct_run_never_skips) {
	char prev_gate[32] = {0};
	char prev_run[32] = {0};
	const char* gate = getenv("SK_CTEST_GATE");
	const char* run = getenv("SK_RUN_INTEGRATION");
	if (gate != NULL) {
		(void)snprintf(prev_gate, sizeof(prev_gate), "%s", gate);
	}
	if (run != NULL) {
		(void)snprintf(prev_run, sizeof(prev_run), "%s", run);
	}

	TEST_ASSERT_EQUAL_INT(0, sk_test_set_env("SK_CTEST_GATE", NULL));
	...
	if (prev_gate[0] != '\0') {
		(void)sk_test_set_env("SK_CTEST_GATE", prev_gate);
	} else {
		(void)sk_test_set_env("SK_CTEST_GATE", NULL);
	}
	if (prev_run[0] != '\0') {
		(void)sk_test_set_env("SK_RUN_INTEGRATION", prev_run);
	} else {
