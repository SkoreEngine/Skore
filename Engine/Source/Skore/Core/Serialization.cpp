...
namespace Skore
{
    ...
    String JsonArchiveWriter::Stringify(ArchiveValue object, bool pretty, bool compressed)
    {
        if (!object) return {};
        ...
        if (char* json = yyjson_mut_val_write_opts(static_cast<yyjson_mut_val*>(object.handler), flg, &alloc, &len, &err))
        {
            ...
            {
                String buffer = {};
                usize bufferSize = ZSTD_compressBound(len);
                buffer.Resize(bufferSize);

                auto size = ZSTD_compress(buffer.begin(), bufferSize, ret.begin(), len, 1);
    ...
    JsonArchiveReader::JsonArchiveReader(StringView string, bool compressed)
    {
        SK_ASSERT(!string.Empty(), "data cannot be empty");

        String buffer = {};
        if (compressed)
        {
            usize size = ZSTD_getFrameContentSize(string.begin(), string.Size());
            buffer.Resize(size);
            usize dSize = ZSTD_decompress(buffer.begin(), buffer.Size(), string.begin(), string.Size());
            string = {buffer.begin(), dSize};
        }
    ...
    ArchiveValue JsonArchiveReader::GetRoot()
    {
