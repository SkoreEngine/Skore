...
#include "Skore/ImGui/Lib/imgui_impl_vulkan.h"
...
namespace Skore
{
    ...
    VulkanDevice::VulkanDevice()
    {
       ...
       enableValidationLayers = true;
        ...
        applicationInfo.pApplicationName = "Skore";
        applicationInfo.applicationVersion = 0;
        applicationInfo.pEngineName = "Skore";
        ...
        if (!Vulkan::QueryInstanceExtensions(requiredExtensions))
        {
            ...
            SK_ASSERT(false, "Required extensions not found");
        }
        ...
        if (vkCreateInstance(&createInfo, nullptr, &instance) != VK_SUCCESS)
        {
            ...
            SK_ASSERT(false, "Error on create vkCreateInstance");
        }

        SK_ASSERT(instance, "instance cannot be created");
    ...
    void VulkanDevice::CreateDevice(Adapter adapter)
    {
        ...
        SK_ASSERT(graphicsFamily != U32_MAX, "Graphics queue not found");
        SK_ASSERT(presentFamily != U32_MAX, "Present queue not found");
        ...
        for (int j = 0; j < SK_FRAMES_IN_FLIGHT; ++j)
        {
    ...
    {
        SK_ASSERT(bufferCreation.size > 0, "size cannot be 0");
        ...
        {
            SK_ASSERT(false, "error on create buffer");
        }
        SK_ASSERT(vulkanBuffer->buffer, "buffer not created");
        return {vulkanBuffer};
    }
    ...
    PipelineState VulkanDevice::CreateGraphicsPipelineState(const GraphicsPipelineCreation& creation)
    {
        ...
        SK_ASSERT(shaderState, "shader is null");
        ...
        SK_ASSERT(!invalidPass, "creation needs attachments or renderpass or pipelineState");
    ...
    RenderPass VulkanDevice::AcquireNextRenderPass(Swapchain swapchain)
    {
        ...
        {
            SK_ASSERT(false, "failed to acquire swap chain image!");
        }
    ...
    void VulkanDevice::EndFrame(Swapchain swapchain)
    {
        ...
        {
            SK_ASSERT(false, "failed to execute vkQueueSubmit");
        }
        ...
        {
            SK_ASSERT(false, "failed to execute vkQueuePresentKHR");
        }
    ...
    {
        SK_ASSERT(bufferDataInfo.data, "data cannot be null");
        SK_ASSERT(bufferDataInfo.size > 0, "size should be higher then zero");
